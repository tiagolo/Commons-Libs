package org.commonsLibs.flex.collections
{
	...
	public class PagedList extends EventDispatcher implements IList
	{
		...
		public function addItemAt(item:Object, index:int):void
		{
			...
			this.data.splice(index, index, item);
		...
		public function getItemAt(index:int, prefetch:int = 0):Object
		{
			...
			else if (item === undefined || item === null)
			{
				...
				const count:int = Math.min(pageSize, this.data.length - pageStartIndex);
		...
		{
			const oldLength:int = this.data.length;
		...
		{
			const rv:Array = new Array(this.data.length);
