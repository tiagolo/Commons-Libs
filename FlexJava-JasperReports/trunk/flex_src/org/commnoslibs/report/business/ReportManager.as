package org.commnoslibs.report.business
{
	...
	public class ReportManager extends AbstractController
	{
		...
		[Bindable]
		public var listaRelatorios:ArrayCollection;
		...
		[Mediate(event = "ReportEvent.DOWNLOAD")]
		...
		[Mediate(event = "ReportEvent.FILL_PARAMETERS", properties = "form,report")]
		...
		[Mediate(event = "ReportEvent.LOGIN_COMPLETE", properties = "userId")]
		public function loginComplete(userId:int):void
		{
			this.userId = userId;
		}

		[Mediate(event = "ReportEvent.REMOVE", properties = "report")]
		...
		public function setReportURL(name:String):void
		{
			...
			{
				var report:Report = getReportFromReportFolder(name, listaRelatorios);
		...
		private function findResults_resultHandler(event:ResultEvent):void
		{
			...
			listaRelatorios = event.result as ArrayCollection;
			listaRelatorios.sort = sort;
			listaRelatorios.refresh();
		}
